...
class Problem(object):
    ...
    def dataset(self,
        ...
        data_filepattern = self.filepattern(
            data_dir, dataset_split, shard=shard)
        ...
        if shuffle_files:
            ...
            for f in data_files:
                ...
                dataset = f_data if dataset is None else dataset.concatenate(
                    f_data)
    ...
    def decode_example(self, serialized_example):
        ...
        data_fields["batch_prediction_key"] = tf.FixedLenFeature([
                                                                 1], tf.int64, 0)
    ...
    def input_fn(self,
        ...
        data_dir = data_dir or (
            hasattr(hparams, "data_dir") and hparams.data_dir)
        ...
        if not batch_size_means_tokens:
            ...
            if config and config.use_tpu:
                ...
                padded_shapes = self._pad_for_tpu(
                    dataset.output_shapes, hparams)
                # on TPU, we use params["batch_size"], which specifies the number of
                # examples across all datashards
                batch_size = params["batch_size"]
                dataset = dataset.apply(
                ...
                if not is_training:
                    ...
                    dataset = dataset.map(
                        _pad_batch, num_parallel_calls=num_threads)
    ...
    def serving_input_fn(self, hparams):
        ...
        dataset = dataset.map(self.decode_example)
        dataset = dataset.map(
            lambda ex: self.preprocess_example(ex, mode, hparams))
    ...
    with tf.name_scope("input_stats"):
        for (k, v) in six.iteritems(features):
            if isinstance(v, tf.Tensor) and v.get_shape().ndims > 1:
                ...
                tf.summary.scalar("%s_nonpadding_tokens" %
                                  k, nonpadding_tokens)
...
class TimeseriesProblem(problem.Problem):
    ...
    def hparams(self, defaults, unused_model_hparams):
        ...
        p.input_modality = {"inputs": (
            registry.Modalities.REAL, self.num_series)}
...
class TimeseriesToyProblem(TimeseriesProblem):
    ...
    def timeseries_dataset(self):
        series = [[float(i + n) for n in range(self.num_series)]
                  for i in range(10)]
